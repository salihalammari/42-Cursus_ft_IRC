...
/*   Updated: 2023/01/18 16:56:10 by schahid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    Client::write(std::string const &message)
{
    ...
    if (send(this->fd, buffer, buffer.length(), 0) < 0)
        //error    
}
...
void Client::join(Channel *channel)
{
    ...
    std::string users;
    std::deque<std::string> nicknames = channel->getNicknames();
    for (std::deque<std::string>::iterator it = nicknames.begin(); it != nicknames.end(); it++)
        users += " ";
